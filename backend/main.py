...
async def verify_key(key: str) -> None:
    if hashlib.sha384(key.encode("utf-8")).hexdigest() != HASHED_KEY:
        raise HTTPException(
            detail="Error, key invalid", status_code=status.HTTP_401_UNAUTHORIZED
        )
...
async def verify_ip(ip: str) -> None:
    if not ip:
        raise HTTPException(
            detail="Error, ip is null", status_code=status.HTTP_422_UNPROCESSABLE_ENTITY
        )
    if not ip.isascii():
        raise HTTPException(
            detail="Error, ip is not ASCII",
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        )
    if len(ip) > 15:
        raise HTTPException(
            detail="Error, ip too long (15 chars max)",
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        )
    if len(ip) < 7:
        raise HTTPException(
            detail="Error, ip too short (7 chars min)",
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        )
    ip_segments: list[str] = ip.split(".")
    if len(ip_segments) != 4:
        raise HTTPException(
            detail="Error, ip must have 4 segments",
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        )

    try:
        ip_segments: list[int] = [int(seg) for seg in ip_segments]
    except ValueError:
        raise HTTPException(
            detail="Error, ip not made of integers",
    ...
    for seg in ip_segments:
        if seg > 255 or seg < 0:
            raise HTTPException(
                detail="Error, ip malformed", status_code=status.HTTP_401_UNAUTHORIZED
            )
...
async def root():
    with open("frontend/index.html", "r") as f:
        data = f.read()
    return HTMLResponse(
        content=data, media_type="text/html", status_code=status.HTTP_200_OK
    )
