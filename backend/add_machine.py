...
class AddMachine:
    ...
    def _create_db(self) -> None:
        with sqlite3.connect(self.filename) as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"CREATE TABLE IF NOT EXISTS {self._table_name} (ip text NOT NULL, mac text NOT NULL)"
            )
    ...
    def add_machine(self, ip: str, mac: str) -> None:
        ...
        with sqlite3.connect(self.filename) as conn:
            ...
            if row:
                cursor.execute(
                    f"UPDATE {self._table_name} SET mac = ? WHERE ip = ?", (mac, ip)
                )
            else:
                cursor.execute(
                    f"INSERT INTO {self._table_name} (ip, mac) VALUES (?, ?)", (ip, mac)
                )
            conn.commit()
